...
# 两个连续次级别 -- 为什么bei不能解释天健集团2015-01-15创新高？
  ...
  (3) 先找买点：下跌后缩量，SCREENER acf即可得之，操作响应周期在1小时线上。
      NMC_RATIO=vol(5)
      PARTS=20
  ...
  (7) 水滴理论: 冲击越大，反弹越大(容器不破时)
      从长期来看，price必定要向avrg靠拢 -- `avrg锚定效应`。
      暴跌选股法 (2015-01-20 002736 国信证券)

# 处理波动 2015-01-21

  不是所有的票都像`乐视网 歌尔声学`一般rdiff呈现完美规律，如`骅威股份`。
  波动会使心理忐忑不安，不患无位，患所以立。

  (1) 双龙战法
      .上个采样周期涨得太快，易导致噪音，两个次级别同时向下形成卖点。
      .分析周期跟随5日线而走。
      - rdiff = diff(ratio)， rdiff < 0 且
      - cdiff = diff(avrg_c)，cdiff < 0 
      - denoise: price(n) > price(n-2)。 

  (2) 振荡位空仓减杠杆，低位满杠杆。

  (3) 多票配置，比价系统票组，票面切换。

  (4) `更广阔的视野，更大的操作格局：操作越频繁，格局越小`，
      [翟鸿燊](http://blog.sina.com.cn/s/blog_80e526b80101iezb.html)
      逸则劳之：用频繁的波动劳散户。
      将军赶路，莫追小兔，将军们都采用`均线交易`。

# 资金

  1 有利可图，资金才会进场
  2 利未到手，资金不会离场
  3 mv=pq, `换手越大`，则资金流入的速度越大 v = s/t = volume/t，流入资金越多
    大换手振荡本身即是最好的资金流入，*流通.大换手.乃资本之生命*
    试图在振荡中计算流入量，及在振荡中赚钱，都只能是徒劳 -- 探路者 2015-01-23
  4 价格低点（-10% of wchng），都是进场资金，持股
    价格高点（+xx% of wchng），尽是离场资金，持币
    价格上升下降，计算净流入及净流出资金。
    全面持股，如ipo，股份便如火箭发射。
    达到利润率，进入横盘整理，并票开始下跌。
  5 最难的是等待与坚持（坚持持股，振荡10日之得利，不如一日暴发的涨幅，这等待便是`格局`）
    -- 坚持每日等待与筛选，不错过任一有机会的票
...
# 大红柱 SELECT get_vol_rat_152468
  ...
  股价底部放量2倍以上，换手3以上，VOL5均线上穿10天线，MACD红色，


# 案例1 中恒集团 vs 康美药业 2015-01-09
  ...
  c) 着眼未来：康美100%换手日量比翻番，但价格未动，而中恒已涨，康美有更大的潜能
